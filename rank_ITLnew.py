...
#        summary_file = '/Users/richard/lsst_archive/u5/mirror/BNL-prod/prod/ITL-CCD/' + ccd + '/' +  str(run) + '/preflight_acq/v0/' + str(job_id) + '/summary.txt'
        ...
        bias = max(np.median(biases[0:7]),np.median(biases[7:]))
	...
	bias_dic={}
        for ccd in ccd_list:
            ...
            bias = self.get_bias(expDict,filePaths, ccd)
            bias_dic[ccd]=bias
...
if __name__ == "__main__":
    ...
    bias_dic, RTM_dic = rank.metric(ccd_list, expDict, filePaths)
    import pickle
    f=open('ITLnewdump.p','wb')
